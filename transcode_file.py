...
'VH265High_ACopy' : './ffmpeg -i {input} -c:a copy -async 1 ' \
                     ...
                     '-pix_fmt yuv420p -crf 18 -maxrate 8M -bufsize 8M -sn -threads {cpu_count} -y {output}',
...
'VH264HighGray_AAAC' : './ffmpeg -i {input} -c:a aac -async 1 ' \
                    '-c:v libx264 -profile:v high422 -preset veryslow ' \
                    '-vf format=gray -pix_fmt yuv420p -crf 18 -maxrate 8M -bufsize 8M -sn -threads {cpu_count} -y {output}',
